param(
  # Denis's base price list. Site prices are the base multiplied by
  # -PriceMultiplier, so the base file stays the single source of truth and no
  # separate "x N" workbook has to be kept in sync.
  [string]$ExcelPath = "G:\Прайс Денис\Прайс на инструмент из наличия на 28 августа 2026.xlsx",
  [string]$OutDir = ".",
  [double]$PriceMultiplier = 2.2,
  # SKUs are carried over from this file by product name. Without it the plain
  # sequential numbering shifts on every added or removed row: regenerating the
  # August list keeps only 66 of 12032 SKUs on the same product, so 12k indexed
  # URLs would quietly start showing different goods. Pass "" to renumber.
  [string]$PreviousData = "data.js",
  # Sheets that are not part of the catalogue. "Калибры" was dropped from the
  # site in July as out of scope; the price list still carries it, and of its
  # ~5200 rows only 11 have a price at all.
  [string[]]$ExcludeSheets = @("Калибры")
)
...
function Convert-ToUa([string]$text) {
  ...
  # Ordered list of pairs, not a hashtable: PowerShell hash keys ignore case, so
  # "Метчики" and "метчики" would collide, while String.Replace below is
  # case-sensitive and needs both spellings.
  $replacements = @(
...
try {
  ...
  # key -> existing SKU, so a product keeps its URL across price-list revisions.
  # The key is category + name + occurrence: the same name legitimately appears
  # in two sheets (gaechnye taps are listed under both their own category and the
  # metric one) and occasionally twice in one sheet. Keying on the name alone
  # collapsed 43 products onto 23 URLs, silently dropping one page of each pair.
  $skuByKey = @{}
  $prevSeen = @{}
  $nextId = 1
  if ($PreviousData -and (Test-Path -LiteralPath $PreviousData)) {
    $prevText = [System.IO.File]::ReadAllText((Resolve-Path -LiteralPath $PreviousData))
    $prevJson = $prevText -replace '^﻿?window\.FLAKS_DATA\s*=\s*', '' -replace ';\s*$', ''
    $prev = $prevJson | ConvertFrom-Json
    foreach ($p in ($prev.products | Sort-Object id)) {
      $base = "$($p.categorySlug)|" + ((([string]$p.nameRu).ToLower() -replace '[\\/]', '/' -replace '\s+', ' ').Trim())
      $n = 0
      if ($prevSeen.ContainsKey($base)) { $n = $prevSeen[$base] }
      $prevSeen[$base] = $n + 1
      $key = "$base|$n"
      if (-not $skuByKey.ContainsKey($key)) { $skuByKey[$key] = $p.sku }
      if ($p.id -ge $nextId) { $nextId = [int]$p.id + 1 }
    }
    Write-Host "Carried over $($skuByKey.Count) SKUs from $PreviousData; new products start at $nextId"
  }
  $seen = @{}
  ...
  foreach ($sheet in $workbook.workbook.sheets.sheet) {
    ...
    if ($ExcludeSheets -contains $sheetName) {
      Write-Host "Skipping sheet '$sheetName' (excluded)"
      continue
    }
    ...
    foreach ($row in $sheetXml.GetElementsByTagName("row")) {
      ...
      if (-not $hasQtyCell -and -not $hasPriceCell) {
        # A handful of product rows carry neither a quantity nor a price and are
        # structurally indistinguishable from a heading. A heading names a group
        # ("Фрезы дисковые отрезные"); a product carries a concrete size, so a
        # diameter, a metric thread or a tooth count rules it out as a heading.
        $looksLikeOneProduct = $name -match "(^|\s)[фФ]\s?\d|(^|\s)[МM]\s?\d+([.,]\d+)?(\s|х|x|$)|z\s?=\s?\d"
        if ($name -notmatch "Наименование|Цена|Кол-во" -and -not $looksLikeOneProduct) {
          $currentSection = $name
        }
        continue
      }
      ...
      # Reuse the SKU this product already had; only genuinely new names get a
      # fresh number, appended above every id ever issued.
      $baseKey = "$catSlug|" + (($nameRu.ToLower() -replace '[\\/]', '/' -replace '\s+', ' ').Trim())
      $occurrence = 0
      if ($seen.ContainsKey($baseKey)) { $occurrence = $seen[$baseKey] }
      $seen[$baseKey] = $occurrence + 1
      $nameKey = "$baseKey|$occurrence"

      if ($skuByKey.ContainsKey($nameKey)) {
        $sku = $skuByKey[$nameKey]
        $productId = [int]($sku -replace '^FLK-', '')
      } else {
        $productId = $nextId
        $sku = ("FLK-" + $productId.ToString("00000"))
        $skuByKey[$nameKey] = $sku
        $nextId++
      }
      ...
      $product = [ordered]@{
        ...
        price = [math]::Round($price * $PriceMultiplier, 2)
